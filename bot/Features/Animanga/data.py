...
async def query_media_recs(
    ...
    Args:
        anilist_id (str): Anilist user ID to query
        media_type (str): Specifies anime or manga statistics
...
async def fetch_recommendations(
    ...
    Args:
        anilist_id (int): Anilist username to query
        media_type (str): Specifies anime or manga statistics
...
async def check_recommendation(
    ...
    Args:
        anilist_id (int): Anilist username to recommend for
    ...
    try:
        time_delta = (
            datetime.now()
            - datetime.strptime(
                known_recs[anilist_id]['date'], '%Y/%m/%d %H:%M:%S'
            )
        ).total_seconds()
    except KeyError:
        time_delta = 0
    if anilist_id not in known_recs or force_update or time_delta > 345600:
        ...
        if media_type.lower() == 'manga':
            bd.known_manga_recs[anilist_id] = {
                'date': datetime.now().strftime('%Y/%m/%d %H:%M:%S'),
                'recs': recommendation_scores,
            }
        else:
            bd.known_anime_recs[anilist_id] = {
                'date': datetime.now().strftime('%Y/%m/%d %H:%M:%S'),
                'recs': recommendation_scores,
            }
    return None
